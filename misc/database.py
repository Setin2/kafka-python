...
class Database():
    ...
    def get_data_by_task_group(self, list_of_tasks):
        """
            A order (group of tasks) will be applied to an image
            Given a list of tasks, we might want the rows where the exact tasks in the list have been applied to the same single image
    ...
    def get_start_time_for_order(self, orderID):
        self.cursor.execute(f"SELECT timestamp FROM metrics WHERE orderID={orderID} LIMIT 1")
